...
from utils.dice_score import jaccard2_loss
...
def train_model(model, device, epochs=5, batch_size=5, learning_rate=1e-5, img_scale=1, rotation=False,
                ...
                dataset_size='1276x664', BGS_input = False, GaussianNoise = False, SpeckleNoise = False, gaussian_var =  0.0001078916145171696, 
    ...
    if TempDim:
        ## Since the cropping is done directlyi n the datset for the 3 frames version, we need to extract the desired shape, we skip the first character as it isnt a number of the shape
   ...
   # This is to print in the training.out file and in the hyperparameter .txt file
    ...
    scheduler = optim.lr_scheduler.ReduceLROnPlateau(optimizer, 'max', patience=patience, min_lr=1e-6) # scheduler to reduce lr and try avoid local min
    ...
    early_stopping_patience = early_stopping_patience  # Set this to your desired patience level for early stopping
    ...
    for epoch in range(1, epochs + 1):
        ...
        with tqdm(total=n_train, desc=f'Epoch {epoch}/{epochs}', unit='img') as pbar:
            for batch in train_loader:
                ...
                with torch.cuda.amp.autocast(enabled=amp):
                    ...
                    loss = jaccard2_loss(target_one_hot[:,1:], input_softmax[:,1:]) #### WE don't take into account the background channel, it would overestimate the performance of the model significantly
       ...
       # conditions to keep at the end of training the best epoch model and also for ealy stopping
    ...
    # COnverting metrics to correct format for recording in .png
   ...
   # Saving best model
    ...
    'Val ': best_val_score.item(),
    'Test': None,
    'Time': elapsed_time
}
    log_training_results('training_log_AllModels.csv', session_data) # This is to save in a .csv all information of models that are being trained
...
if __name__ == '__main__':
    ...
    # THIS except isn't really ever used, it can be deleted
