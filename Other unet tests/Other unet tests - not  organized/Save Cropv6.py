...
def process_videos(dataset_type, window_height, window_width):
    dataset_path = os.path.join(r'Small_ARIS_Mauzac_UnetReady_Final_BGS', dataset_type)
    ...
    for video_path in videos:
        ...
        bgs_paths = glob.glob(os.path.join(video_path, "BGS", "t*"))

        if len(masks_paths) != len(originals_paths) or len(masks_paths) != len(bgs_paths):
            print(f"Error : Video in {dataset_type} doesn't have the same tracks for gt, input, and BGS")
            break
        ...
        for i in range(len(masks_paths)):
            ...
            bgs_in_track = glob.glob(os.path.join(bgs_paths[i], "BGS_*"))
            sorted_bgs_in_track = sorted(bgs_in_track, key=extract_frame_number)

            if len(sorted_masks_in_track) != len(sorted_originals_in_track) or len(sorted_masks_in_track) != len(sorted_bgs_in_track):
                print(f"Error : Video in {dataset_type} doesn't have the same number of frames for gt, input, and BGS")
                break
            ...
            for j in range(len(sorted_masks_in_track)):
                ...
                resized_bgs = crop(bgs_image, rmin, rmax, cmin, cmax, window_height, window_width)

                # Output paths for cropped images
                save_base_path = os.path.join(r"..\Pytorch-UNet-masterv2", f'D{window_height}x{window_width}\data_{dataset_type.lower()}')

                #mask_filename
                mask_filenameSplit = mask_filename.split("_")
                original_filenameSplit = os.path.basename(sorted_originals_in_track[j]).split("_")
                bgs_filenameSplit = os.path.basename(sorted_bgs_in_track[j]).split("_")
                if mask_filenameSplit[2] == "Arcing":
                    masks_save_path = os.path.join(save_base_path, str(mask_filenameSplit[3]+"_"+ mask_filenameSplit[4]) , str(mask_filenameSplit[5]), "masks")
                else:
                    masks_save_path = os.path.join(save_base_path, str(mask_filenameSplit[2]+"_"+ mask_filenameSplit[3]), str(mask_filenameSplit[4]), "masks")
                images_save_path = os.path.join(save_base_path, str(original_filenameSplit[0]+"_"+ original_filenameSplit[1]), str(original_filenameSplit[2]),"imgs")
                bgs_save_path = os.path.join(save_base_path, str(bgs_filenameSplit[1]+"_"+ bgs_filenameSplit[2]), str(bgs_filenameSplit[3]), "bgs") 
                # 2014-11-14_100000_t0_Obj_frame2798.
                os.makedirs(masks_save_path, exist_ok=True)
                os.makedirs(images_save_path, exist_ok=True)
                os.makedirs(bgs_save_path, exist_ok=True)
