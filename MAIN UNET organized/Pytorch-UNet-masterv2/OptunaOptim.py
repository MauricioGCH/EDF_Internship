...
def objective(trial):
    ...
    # Here you modify your search space
    config = {
    ...
    for epoch in range(1, config["epochs"] + 1):
        ...
        with torch.autocast(device.type if device.type != 'mps' else 'cpu', enabled=config["amp"]):
            for batch_idx, batch in enumerate(tqdm(val_loader, total=num_val_batches, desc='Validation round', unit='batch', leave=True)):
                ...
                jaccard_score += jaccard2_coef(mask_true[:,1:], mask_pred[:,1:]).item() # leave out the background
        ...
        trial.report(val_score, epoch) # HERE IS WHERE THE MODEL INFORMATION IS REPORTED TO OPTUNA
        if trial.should_prune():
            raise optuna.exceptions.TrialPruned()
            
        # This is used to record this extract parameters from the current trial in the study
...
if __name__ == "__main__":
    ...
    study.optimize(objective, n_trials=20) # HERE YOU GIVE YOUR OBJECTIVE FUNCTION AND MODIFY THE NUMBER OF TRIALS PER STUDY. You can set the trials as a parameter for an easier manipulation.
    ...
    print("config parameters search space : ")
