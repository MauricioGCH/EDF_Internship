...
class BGBasicDataset(Dataset):
    def __init__(self, images_dir: str, mask_dir: str, bg_dir: str, scale: float = 1.0, mask_suffix: str = '', rotation: bool = False, vertical_flip: bool = False, contrast: bool = False, gaussian: bool = False, speckle: bool = False, gaussian_var: float =  0.001, speckle_factor : float =  0.1, contrast_range: tuple = (1, 1.5), rot_degree: int = 10):
    ...
    def __getitem__(self, idx):
        ...
        bg_img_file = list(self.bg_dir.glob('crop_BGS_' + NoCropName + '.*'))
        namesearch = str((str(self.bg_dir) + '/crop_BGS_' + NoCropName + '.*'))

        assert len(img_file) == 1, f'Either no image or multiple images found for the ID {name}: {img_file}'
        assert len(bg_img_file) == 1, f'Either no background subtraction image or multiple images found for the ID {name}: {bg_img_file}, the name search was {namesearch}'
        assert len(mask_file) == 1, f'Either no mask or multiple masks found for the ID {name}: {mask_file}'
       
        mask = load_image(mask_file[0])
        image = load_image(img_file[0])
        bg_img = load_image(bg_img_file[0])
       
        assert image.size == mask.size == bg_img.size, \
            f'Image, background subtraction image, and mask {name} should be the same size, but are {image.size}, {bg_img.size}, and {mask.size}'
       

       
        if self.contrast:
            image = random_contrast(image, contrast_range = self.contrast_range)
        if self.gaussian:
            image = add_gaussian_noise(image, var = self.gaussian_var)
        if self.speckle:
            image = add_speckle_noise(image, noise_factor = self.speckle_factor)
       
       
        images = [image.copy(), bg_img.copy()]
        ...
        images = [self.preprocess(images[0], self.scale, is_mask=False), (self.preprocess(images[1], self.scale, is_mask=False)).astype(np.uint8)]
        mask = self.preprocess(mask, self.scale, is_mask=True)

       

        #images = np.stack((np.array(images), np.array(bg_img)), axis=0)
        combined_img = np.concatenate(images, axis=0)
       

        #images = self.preprocess(images, self.scale, is_mask=False)
        #images = [self.preprocess(img, self.scale, is_mask=False) for img in images]
        ...
        return {
            'image': torch.as_tensor(combined_img.copy()).float().contiguous(),
            'mask': torch.as_tensor(mask.copy()).long().contiguous(),
            'file_paths': {
                'img_file': str(img_file[0]),
                'bg_img_file': str(bg_img_file[0]),
    ...
    def visualize_sample(self, idx): # This was added so i could directly watch if the images were being loaded correctly, as well as if the transformation were done correctly
        ...
        combined_image = sample['image'].numpy()
        mask = sample['mask'].numpy()
        file_paths = sample['file_paths']
       
        original_image = combined_image[0, :, :]  # Extract the first channel (original image)
        bg_image = combined_image[1, :, :]  # Extract the second channel (background subtraction image)
        ...
        axs[1].imshow(bg_image, cmap='gray')
        axs[1].set_title('Background Subtraction Image')
        axs[1].axis('off')

        # Stack the images along the last dimension to create a 3-channel image
        stacked_image = np.stack((original_image, bg_image, np.zeros_like(original_image)), axis=-1)
        axs[2].imshow(stacked_image)
        axs[2].set_title('Stacked Image')
        axs[2].axis('off')
        ...
        print(f"Image file: {file_paths['img_file']}")
        print(f"Background subtraction image file: {file_paths['bg_img_file']}")
