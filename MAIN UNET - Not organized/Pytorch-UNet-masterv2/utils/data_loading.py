...
def load_image(filename):
    ...
    if ext == '.npy':
        ...
        if "_m_" in filenamepath: # its a mask
            ...
            # 3 is Eel arcing, 2 is smallfish arcing, 1 is trash arcing, 4 is smallfish, 5 is trash, 6
            #maskFINAL = np.zeros((mask.shape[0], mask.shape[1], 6 + 1), dtype=np.uint8)

            #maskFINAL[:,:, 0] = np.where(mask == 1, 0, 1) #background
            #print("wait")
...
class BasicDataset(Dataset):
    def __init__(self, images_dir: str, mask_dir: str, scale: float = 1.0, mask_suffix: str = '', rotation: bool = False, 
        ...
        #self.ids = [splitext(file)[0] for file in listdir(images_dir) if isfile(join(images_dir, file)) and not file.startswith('.')].sort()
    ...
    def preprocess(np_img, scale, is_mask):
        ...
        #np_img = cv2.resize(np_img, (newW, newH),  interpolation = cv2.INTER_NEAREST if is_mask else cv2.INTER_CUBIC)
