...
def visualize_sample(images, masks, outputs, epoch, batch_idx, VisualizationProb, division, n_classes, session_folder):
    ...
    if division == "test":
        fig, axes = plt.subplots(3, int(images.shape[0]), figsize=(15, 15))
...
def evaluate(net, dataloader, device, amp, epoch, session_folder, log_interval=10, mask_threshold=0.5):
    ...
    with torch.autocast(device.type if device.type != 'mps' else 'cpu', enabled=amp):
        for batch_idx, batch in enumerate(tqdm(dataloader, total=num_val_batches, desc='Validation round', unit='batch', leave=False)):
            ...
            # Collect all predictions and targets for classification report
            if net.n_classes == 1:
                mask_pred = (F.sigmoid(mask_pred) > mask_threshold).float()
            else:
                mask_pred = F.one_hot(mask_pred.argmax(dim=1), net.n_classes).permute(0, 3, 1, 2).float()

            #all_preds.extend(mask_pred.flatten().cpu().numpy())
            #all_targets.extend(mask_true.flatten().cpu().numpy())

            # Visualize sample
            visualize_sample(image, mask_true, mask_pred, epoch, batch_idx, mask_threshold, "test", n_classes=net.n_classes, session_folder=session_folder)
