...
def train_model(model, device, epochs=5, batch_size=5, learning_rate=1e-5, img_scale=1, rotation=False,
                verticalFlip=False, contrast=False, TempDim=False, amp=False, weight_decay=1e-8, patience=100, early_stopping_patience=50, gradient_clipping=1.0,
                dataset_size='350x350', ModLoss = False, BGS_input = False, GaussianNoise = False, SpeckleNoise = False):
    ...
    if not TempDim:
        dir_img = script_dir / f'{dataset_size}/data_train/imgs/'
        dir_mask = script_dir / f'{dataset_size}/data_train/masks/'
    else: 
        dir_img = script_dir /'FullSize1276x664/data_train/imgs/'
        dir_mask = script_dir /'FullSize1276x664/data_train/masks/'
    if BGS_input:
        dir_bgs = script_dir /f'{dataset_size}/data_train/bgs/'
    ...
    if not TempDim:
        dir_img_val = script_dir /f'{dataset_size}/data_val/imgs/'
        dir_mask_val = script_dir /f'{dataset_size}/data_val/masks/'
    else:
        dir_img_val = script_dir /'FullSize1276x664/data_val/imgs/'
        dir_mask_val = script_dir /'FullSize1276x664/data_val/masks/'
        
    if BGS_input:
        dir_bgs_val = script_dir /f'{dataset_size}/data_val/bgs/'
    ...
    for epoch in range(1, epochs + 1):
        ...
        with tqdm(total=n_train, desc=f'Epoch {epoch}/{epochs}', unit='img') as pbar:
            for batch in train_loader:
                ...
                with torch.cuda.amp.autocast(enabled=amp):
                    ...
                    loss = jaccard2_loss(target_one_hot, input_softmax) ####
        ...
        train.report({"Val_score": val_score.cpu().numpy()}, checkpoint=None)
        #tune.report(loss=epoch_loss / train_count, val_score=val_score)
        tune.report(val_score=val_score.cpu().numpy())
    ...
    return best_model, best_val_score.cpu().numpy()
